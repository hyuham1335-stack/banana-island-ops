...
계획 → 교차검증 → 계약 확정 → 구현 → 게이트(테스트·린트) → 코드 리뷰 → PR 생성 → 보고
...
| **페이즈**(phase) | 런을 나눈 8개 단계입니다. 각 단계의 요구와 산출물은 `harness/phases/01~08.md` 에 있습니다 |
...
flowchart TD
    ...
    P02["02 교차검증<br/>계획서 전문을 다른 모델로 검증"]
    P03["03 구현<br/>계약 확정 · 구현·테스트 병렬 작성"]
    P04["04 게이트<br/>스테이지 실행 · 실패 귀속"]
    P05["05 코드리뷰<br/>계약 대조 · 리뷰어 병렬 호출"]
    P06["06 PR<br/>승인 기록 · push · PR 본문"]
    P07["07 PR 리뷰<br/>외부 리뷰 수집 · 규칙 승격"]
    P08["08 보고<br/>런 보고서 작성"]
    DONE(["exit 11 · 런 완료<br/>PR 까지. 머지는 하지 않음"])
    ASK(["exit 9<br/>사람의 판단 대기"])
    STOP(["exit 5 · 7 · 10<br/>에스컬레이션"])

    P01 --> P02 --> P03 --> P04 --> P05 --> P06 --> P07 --> P08 --> DONE
    P02 -. "critical 잔존 시 1회 반려" .-> P01
    P04 -. "실패 시 담당 역할이 수리 (최대 3회)" .-> P04
    P05 -. "지적 반영 후 재검토 (최대 2회)" .-> P05
    P07 -. "변경 요청이 열려 있으면" .-> P07
    P06 -. "승인이 없거나 원격이 없으면" .-> ASK
    P04 -. "수리 기회 소진 또는 동일 실패 반복" .-> STOP
```

| 단계 | 무엇을 하나 | 무엇이 나오나 | 언제 통과하고 언제 멈추나 |
|---|---|---|---|
| **01 계획** | 사용자의 원본 요청을 동결하고, 그 요청을 빠짐없이 덮는 계획서를 씁니다. 계획의 각 항목에는 요청 원문에서 그대로 따온 문장을 근거로 붙입니다 | 계획서와 회차별 리뷰 기록 | 근거가 요청 원문의 부분문자열인지, 요청의 모든 항목이 정확히 한 번씩 덮였는지를 봅니다. 반복해도 수렴하지 않으면 에스컬레이션합니다 |
| **02 교차검증** | 완성된 계획서 전문을 교차검증기에 넘깁니다. 01 의 리뷰는 매 회차의 계획만 봤고 완성본 전체를 본 적이 없기 때문입니다. 교차검증기는 외부 플랜 리뷰 도구(다른 모델)가 primary 이고, 없으면 `plan-reviewer` 에이전트로 폴백합니다 | 판정 JSON | `critical` 이 0건이어야 넘어갑니다. 남으면 01 로 1회 반려합니다. 둘 다 불가한 환경이면 스킵하되 등급을 `PASS_WITH_GAPS` 로 낮춥니다 |
| **03 구현** | 이번에 만들 유닛과 진입점 목록(계약)을 먼저 확정하고, 역할들이 **병렬로** 각자의 경로를 채웁니다. 만들 것을 미리 못 박기 때문에 구현과 테스트가 서로를 보고 형태를 맞추는 일이 없습니다 | 계약 문서와 역할별 작업 목록 | 소유 경계를 넘은 파일이나 주인 없는 파일이 없어야 하고, 컴파일·타입 검사를 통과해야 합니다 |
| **04 게이트** | 어댑터에 적힌 스테이지를 순서대로 실행하고, 실패를 귀속시켜 해당 역할에만 되돌립니다 | 스테이지 결과와 귀속 결과 | 수리 기회는 3회입니다. 기회가 남아도 **같은 역할이 같은 실패를 두 번 내면** 에스컬레이션합니다. 테스트 0개는 기록만 하고 막지 않지만, 기존 테스트 수가 크게 줄면 막습니다 |
| **05 코드리뷰** | 04 가 "동작하는가" 를 봤다면 05 는 "계약대로 만들었는가, 봐야 할 리뷰어가 실제로 봤는가" 를 봅니다. 비용이 들지 않는 `precheck` 부터 돌리고, 변경된 파일에 해당하는 리뷰어만 병렬로 부릅니다 | 계약 대조 결과, 리뷰 결과, 승격 후보 | 재검토는 2회까지입니다. 리뷰어가 전부 실패했거나 호출된 리뷰어가 0명이면 등급이 `PASS_WITH_GAPS` 로 내려갑니다 |
| **06 PR** | 승인을 받아 기록으로 남기고, 계약 문서를 삭제한 뒤 원격에 push 하고, PR 본문을 만듭니다. 실제 PR 생성은 메인 세션이 GitHub 도구로 합니다 | PR 본문과 PR 생성 요청서 | 승인이 없으면 exit 9 로 정상 종료하고 사람을 기다립니다. 승인 후 코드가 바뀌면 다시 승인받습니다. 원격과 어긋나면 강제로 덮어쓰지 않고 멈춥니다 |
| **07 PR 리뷰** | 05 가 우리 코드를 우리가 본 것이라면 07 은 **외부 리뷰**를 받는 단계입니다. 외부 리뷰를 모으고, 원장에 쌓인 지적 중 기준을 넘은 것을 자동 검사 규칙으로 승격합니다 | PR 리뷰 결과와 승격 결과 | 변경 요청이 열려 있으면 멈춥니다. 재검토는 2회까지입니다 |
...
| 경로 | 무엇 |
|---|---|
| `config.json` | 이 프로젝트의 설정 하나입니다. 어댑터, 역할과 소유 경계, 계약 문서의 절 제목, 리뷰어 호출 조건, 런당 예산(파일 수·줄 수·모델 호출 수), 브랜치 규칙이 들어갑니다 |
| `config.schema.json` | 위 파일의 스키마입니다. 검사기는 표준 라이브러리로 직접 만들었고, `_` 로 시작하는 키는 주석으로 보고 건너뜁니다 |
...
| `profiles/<어댑터>/config.json` | 미리 채워 둔 설정 프로필입니다. `harness.py init` 이 복사하면서 프로젝트 이름만 바꿔 넣습니다 |
...
| `pipeline/attribution.py` | 실패의 귀속을 가려냅니다. 파일도 프로세스도 건드리지 않는 순수 함수라, 실패 상황을 저장해 두었다가 그대로 재현할 수 있습니다 |
...
| `pipeline/review.py` | 변경된 파일을 보고 호출할 리뷰어를 정합니다. **자기가 쓴 코드를 자기가 리뷰할 수 없다**는 규칙을 여기서 강제합니다 |
| `pipeline/precheck.py` | 05 에서 가장 먼저 도는 무비용 검사입니다. 예산, 브랜치, 기준 브랜치와의 차이, 환경을 봅니다. 모델도 테스트도 부르지 않습니다 |
...
| `pipeline/review07.py` | 07 에서 내장 리뷰의 호출 여부와 강도를 정해진 규칙으로 정합니다 |
| `pipeline/ledger.py` | 지적 기록을 읽고 씁니다. **읽고 쓰기만 하고 판단하지 않습니다** |
| `pipeline/promote.py` | 쌓인 지적을 자동 검사 규칙으로 올립니다. 실제 적용은 07 에서 런당 한 번뿐입니다 |
...
| `session_log.py` | 세션 종료 훅이 부르는 기록기입니다. **해석 없이 사실만 남깁니다.** 실패해도 세션을 막지 않고, 실패했다는 사실을 기록에 남깁니다 |
...
| 경로 | 무엇 |
|---|---|
| `commands/feature.md` | `/feature` 명령입니다. `doctor` 로 열고, 사용자의 요청을 한 글자도 바꾸지 않고 동결한 다음, 종료 코드에 따라 다음 행동을 정합니다 |
...
| 명령 | 하는 일 |
|---|---|
| `doctor` | 설정 검사에 더해 파이프라인 쪽 7가지를 봅니다(런 폴더가 git 에서 제외됐는지, 에이전트 정의가 있는지, 페이즈 정의가 온전한지, 계약 틀이 파싱되는지, 리뷰어 설정이 맞는지, 원격과 기준 브랜치가 있는지, 외부 리뷰 봇 설정이 맞는지) |
| `init --feature <이름> --request-file <경로>` | 런을 만들고, 요청 파일을 한 글자도 바꾸지 않은 채로 동결합니다 |
