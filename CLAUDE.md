...
> 실패다. 규칙을 늘리는 것보다 **워커가 그것을 읽었는지 게이트가 묻게** 하는 쪽이 낫다.
...
- CRITICAL: 테스트를 먼저 작성하고 그것을 통과하는 구현을 작성할 것 (TDD). **버그 수정도 같다** — 재현 테스트 → 실패 확인 → 수정 → 재검증. 테스트 범위와 커버리지 기준은 `/docs/TRD.md`를 따른다. 테스트는 **대상 줄을 지우면 빨간불이 되는가**로 검증한다 — DB 스텁은 `.where` 등 조건 인자를 기록해 단언하고, 실패 경로는 동기 throw 와 await 시점 reject 를 둘 다, 분기는 포함·배제·`null` 을 모두 잠근다
