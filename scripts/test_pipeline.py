...
class TestRealTscOutput:
    """보관된 **실물** tsc 출력 한 줄. 지어낸 실패가 아니다 (ROADMAP §6).

    런 `20260915-1754-5568` 의 `gates/03_compile.log` (exit 2) 에서 그대로
    옮겼다. `_workspace/` 는 비추적이라 그 파일에 기대지 않고 줄을 여기 고정한다.
    2026-09-03 에 이 정규식이 실물 출력에 **0건 매칭**이던 적이 있고, 그 사실은
    픽스처로는 드러나지 않았다 ([[ADR-H069]]).
    """

    # tsc 6.x · next typegen 경유. 형식은 `path(line,col): error TSxxxx: …` 다.
    REAL = ("src/services/content-generation.test.ts(659,12): error TS18047: "
            "'result.data.ruleSnapshot' is possibly 'null'.")

    def test_실물_tsc_줄에서_경로와_위치를_뽑는다(self, repo, config):
        _c, adapter, _cal = adapters.load(repo)
        got = attr.attribute_compile(adapter, config, set(), self.REAL)
        assert len(got) == 1, "한 줄이면 한 건이다"
        assert got[0]["file"] == "src/services/content-generation.test.ts"
        assert got[0]["owner_reason"] == "compile_error_regex → owns"

    def test_실물_줄이_테스트_역할로_간다(self, repo, config):
        """경로가 테스트 파일이고 계약 심볼 부재가 아니므로 test 가 맞다."""
        _c, adapter, _cal = adapters.load(repo)
        got = attr.attribute_compile(adapter, config, set(), self.REAL)
        assert got[0]["owner"] == "test"
        assert attr.rules_fired(adapter, got) == {"compile_error_regex"}
...
class TestPrecheckBudget:
    ...
    def test_too_many_lines_is_exit_9(self, repo):
        _branch(repo, "feat-x")
        _bulk_change(repo, 1, lines=1600)   # budget.lines_max 는 1500 이다
        got = pc.run(repo, scope="pr")
        assert got["exit"] == 9
