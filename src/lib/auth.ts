 ...
 *
 * `getServerActor` 는 계약(run 20260916-0038-3305)에서 추가 — 서버 컴포넌트·레이아웃이
 * next/headers 의 쿠키 스토어로 같은 값을 읽는다. DB·다른 I/O 없음.
 */
...
export function resolveRole(value: string | undefined): Actor["role"] {
...
export function getActor(request: Request): Actor {
  ...
  return { role: resolveRole(match ? match[1] : undefined) };
}

export async function getServerActor(): Promise<Actor> {
  const store = await cookies();
  return { role: resolveRole(store.get("role")?.value) };
}
