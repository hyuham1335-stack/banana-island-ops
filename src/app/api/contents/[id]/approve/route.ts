...
import { ApproveInputSchema } from "@/lib/schemas";
import { approveContent } from "@/services/content-workflow";

/**
 * POST /api/contents/{id}/approve — FR-010 계약(run 20260915-2042-728c).
 * admin 만 허용 — approveContent → transition() 내부의 FORBIDDEN_ROLE 판정이 유일한
 * 인가 지점이다(CLAUDE.md — 화면에서 버튼을 숨기는 것은 인가가 아니다, 서버에서 검사한다).
 */
