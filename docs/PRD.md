...
- And URL 이 있으면 HEAD 로 5초 안에 확인한다. 확인되면(`ok`) 「링크 확인됨」을 표시하고 발행 완료한다. 확인 실패(`unreachable`)면 발행 완료는 되지 않고 콘텐츠는 `approved` 로 남는다 — 담당자는 URL 을 고치거나 지운 뒤 다시 시도한다
...
| 발행 URL HEAD 실패 | 발행 완료 차단(422 `URL_UNREACHABLE`), `approved` 유지 | 「URL 을 확인할 수 없어 발행을 완료하지 못했습니다」 |
