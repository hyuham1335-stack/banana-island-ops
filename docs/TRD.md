...
| FR-007 | `regenerate(llm, contentId, instruction)` — 현재 본문을 `content_history` 로 복사(`reason='regenerate'`) 후 FR-005 ⑤~⑦ 재실행(instruction 을 user 끝에 추가). `regen_count += 1`. 상태는 `draft` 유지(`rejected` 면 `draft` 로) | `services/content-generation.ts` |
...
| FR-009 · FR-010 · FR-011 · FR-013 · FR-014 | `transition(contentId, action, actor)` 하나가 상태 기계 표를 보고 허용 여부·다음 상태·부수효과(계획 연결 이전, 예시 등록, 이력, 시각 컬럼)를 처리한다. 표에 없는 조합은 `INVALID_TRANSITION`. admin 전용 액션은 `actor.role !== 'admin'` 이면 `FORBIDDEN_ROLE`. `submit` 은 `detected_terms.blocks.length === 0` 필수(`BLOCKED_TERMS_REMAIN`) | `services/content-workflow.ts` |
